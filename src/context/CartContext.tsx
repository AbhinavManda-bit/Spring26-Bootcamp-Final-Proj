...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    const loadCart = async () => {
      ...
      if (thisUsersCartItems) {
        ...
        setItems([]); // ← change null to [] for logged-in buyers
      }
  ...
  const addItem = async (productId: string) => {
    ...
    if (!(await verifyItemIdInCatalog(productId))) {
      setLoadingCart(false);
      throw new Error(
        "Item with this product id is not in the list of products (product catalog).",
      );
    }
    if (currentUser) {
      ...
      if(items === null) throw new Error("Cannot add an item to a null cart.");
  ...
  const calcTotalPrice = async () => {
    ...
    if (currentUser) {
      ...
      if (currentUserCart) {
        ...
        setLoadingCart(false);
        throw new Error(
          "Could not fetch this users cart. " +
